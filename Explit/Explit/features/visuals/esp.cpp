...
void c_esp::start()
{
	...
	{
		for (int i = 1; i < g_interfaces.p_entitylist->get_highest_entity_index(); i++) {
			const auto entity = reinterpret_cast<c_base_entity*>(g_interfaces.p_entitylist->get_client_entity((i)));
			if (!entity)
				continue;
			if (entity->is_dormant())
				continue;
			if (entity == g_interfaces.g_localplayer && !g_config.settings.visuals.esp.local)
				continue;
			...
			if (g_config.settings.visuals.esp.enemies && entity->is_valid()) draw_players(entity, box);
		}
	}
...
void c_esp::draw_players(c_base_entity* pplayer, RECT box)
{
	...
	{
		if (pplayer != g_interfaces.g_localplayer)
		{
